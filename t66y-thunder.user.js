...
// @version      1.0
// @description  左手大拇指优化：完美绕过rmdown广告拦截机制，零延迟直达迅雷下载
// @author       Gemini
...
(function () {
  ...
  if (currentUrl.includes("rmdown.com")) {
    ...
    if (urlParams.get("auto_thunder") === "1") {
      // 1. 降维打击：直接空接全局 window.open，让原页面的广告弹窗原地蒸发
      window.open = function () {
        console.log("油猴助手：已成功拦截流氓广告弹窗");
        return null;
      };

      // 2. 函数劫持：直接覆盖原页面的 magnet_decider，干掉 1000ms 延迟，实现动态磁力秒唤起
      window.magnet_decider = function (data, copy, cbtn) {
        if (!copy && data) {
          console.log("油猴助手：已成功抓取真实磁力，正在唤起迅雷...");
          window.location.href = data; // 零延迟直接变轨到 magnet 协议
        }
      };

      // 3. 自动点火：等待页面元素就绪后自动触发点击
      const triggerClick = () => {
        const magnetBtn =
      ...
      // 考虑到手机渲染性能，稍微给 150ms 缓冲确保原页面的 jQuery 绑定事件已就绪
